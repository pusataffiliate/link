...
const DEFAULT_FIREBASE_CONFIG = {
  apiKey: "AIzaSyBQM4NQ_8U4d69Otsvkoea76sve0hQq9Mw",
  authDomain: "linkproduct99.firebaseapp.com",
  databaseURL: "https://linkproduct99-default-rtdb.asia-southeast1.firebasedatabase.app",
  projectId: "linkproduct99",
  storageBucket: "linkproduct99.firebasestorage.app",
  messagingSenderId: "190854015809",
  appId: "1:190854015809:web:de6763df391778a0ee4fc0",
  measurementId: "G-4VZQ8JLSM0"
};
...
class DatabaseManager {
  ...
  getSavedConfig() {
    try {
      ...
      if (stored) {
        ...
        if (parsed && parsed.apiKey && (parsed.databaseURL || parsed.projectId)) {
  ...
  async init() {
    ...
    if (config && config.apiKey && (config.databaseURL || config.projectId) && window.firebase) {
      try {
        ...
        console.log("✅ Firebase successfully initialized with default credentials.");
      } catch (err) {
    ...
    // Initialize default local storage data if not present and Firebase is not ready
      ...
      try {
        ...
        if (snapshot.exists()) {
          ...
          try {
            ...
            console.warn("Could not seed default profile to Firebase:", err);
          }
          localStorage.setItem('store_profile', JSON.stringify(DEFAULT_STORE_PROFILE));
  ...
  async getProducts() {
    ...
    const products = local ? JSON.parse(local) : (this.isFirebaseReady ? [] : DEFAULT_PRODUCTS);
